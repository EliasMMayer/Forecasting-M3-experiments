...
fc_es_tr <- forecast(fit_ets_train, h = 8, level = .95)
fc_es_MAda_tr <- forecast(fit_ets_MAdA_train, h = 8, level = .95)


#Accuracy tests

ac_MMA<- fc_es_tr$mean %>% accuracy(y_train_test_ts)
ac_MAdA<- fc_es_MAda_tr$mean  %>% accuracy(y_train_test_ts)
...
for (origin in origins){
  ...
  fc_2<- forecast::forecast(fit_2, h=h) 
...
fc_es_MMA <- forecast(fit_ets, h = 8, level = .95)
fc_es_MMA_lower <- forecast(fit_ets, h = 8, level = .80)
```
...
a_MMA <- fc_es_MMA$mean %>% accuracy(y_test)

a_Bench <-comp_fit %>% accuracy(y_test)
