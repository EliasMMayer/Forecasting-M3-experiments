...
for (origin in origins){
  ...
  fit <- forecast::auto.arima(yt) #first model performance
...
colnames(arr_all) <- c("Series_ID", "n_observations", "type", "period", "MAPE_ARIMA", "sMAPE_ARIMA", "CV_MAPE_ARIMA", "model", "CV_MAPE_NAIVE", "CV_MAPE_MEAN", "CV_MAPE_SNAIVE", "Trend")
...
for (nameId in arr_ids) {
  ...
  ets_fit <- forecast::auto.arima(y_training)
  ...
  arr_all[count,7] <- crossVal_ETS(y_training)
  
  
  #
  
  
  arr_all[count,8] <- ets_fit$arma
...
#write_xlsx(arr_tsb,"C:\\Users\\elias\\Desktop\\tab2.xlsx")
